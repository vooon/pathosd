...
func TestValidate_IPv6VIP(t *testing.T) {
	t.Run("IPv6 VIP requires IPv6 local_address", func(t *testing.T) {
		cfg := validConfig()
		cfg.VIPs[0].Prefix = "2001:db8::1/128"
		cfg.VIPs[0].Check.HTTP.Host = "2001:db8::1"
		cfg.Router.LocalAddress = "" // not set
		assertErrorContains(t, Validate(cfg), "router.local_address")
	})

	t.Run("IPv6 VIP rejected with IPv4 local_address", func(t *testing.T) {
		cfg := validConfig()
		cfg.VIPs[0].Prefix = "2001:db8::1/128"
		cfg.VIPs[0].Check.HTTP.Host = "2001:db8::1"
		cfg.Router.LocalAddress = "192.168.1.1"
	...
	t.Run("IPv6 VIP with IPv6 local_address is valid", func(t *testing.T) {
		cfg := validConfig()
		cfg.VIPs[0].Prefix = "2001:db8::1/128"
		cfg.VIPs[0].Check.HTTP.Host = "2001:db8::1"
		cfg.Router.LocalAddress = "2001:db8::2"
		assert.Empty(t, Validate(cfg))
	})
}
